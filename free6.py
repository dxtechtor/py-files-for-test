...
BOT_TOKEN = "8733139125:AAFr4SKKtv-Zl_fLDm5f3DOfgeNrm4aZRUM"
BOT_OWNER = 8626582205
# _API_KEY = "a2fde61ac44edacd86495d19a93ce63e6a78a297039aa764a8d550f3c458afb0"
_API_KEY = "FREE_DDOSBYDX"
#_API_KEY = "pro"
#_API_KEY = "teat"
_API_URL = "http://professorxme.site/api.php"
...
def create_ghopghop_gif(bot, user_id, user_name):
    gif = Image.open("ghopghop_template.gif")
    
    # 1. FETCH REAL-TIME FULL NAME
    # This ensures we get the name even if they aren't in your database
    try:
        chat_member = bot.get_chat_member(bot.get_chat(user_id).id, user_id) # Attempt to fetch
        real_name = chat_member.user.first_name
        if chat_member.user.last_name:
            real_name += " " + chat_member.user.last_name
    except:
        # Fallback to the name passed in the command if API call fails
        real_name = user_name

    # 2. Setup Frames and Font
    frames = []
    try:
        # Update this path to match your server's font location
    ...
    for frame_index in range(gif.n_frames):
        ...
        # 4. Draw Full Name
        # Coordinates (150, 160) - Adjust these to fit your GIF
        draw.text((110, 130), str(real_name), fill="white", font=font)
...
def build_global_status_message(user_id):
    ...
    if active_attacks:
        ...
        response += "💤 use ''/faaa ip port time'' to attack\n"
        response += "────────────────────────\n"
    ...
    if free_slots <= 0:
        ...
        response += f"<blockquote>🚀 Tired of waiting? Bypass slots and get instant power! Contact @dxtechtor_owner to buy premium.</blockquote>\n"
...
def handle_attack(message):
    ...
    # Only check if the system is enabled AND the user has pending feedback
    ...
    try:
        ...
        # INSIDE your /faaa function, after successful validation:
        ...
        with _attack_lock:
            ...
            # Look for this inside your /faaa handler
        username = message.from_user.username
...
def message_ban_command(message):
    ...
    try:
        ...
        # Generate and Send GIF
                # Generate the GIF buffer using the Pillow logic previously provided
        gif_buffer = create_ghopghop_gif(bot, target_user_id, resolved_name or "User")
        
        # Ensure the buffer name is explicitly set to .gif
...
def unified_group_join_handler(message):
    ...
    for new_user in message.new_chat_members:
        ...
        welcome_text = f"Hello {clean_name}, welcome to the Universe of DX."
