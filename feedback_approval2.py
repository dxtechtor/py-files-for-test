...
FEEDBACK_DB_PATH = "bot_data_free/pending_feedback.json"
ADMIN_ID = 8626582205
bot = telebot.TeleBot(TOKEN)
# Constants for persistent storage
SETTINGS_PATH = "bot_data_free/settings.json"
ALLOWED_REVIEWERS_PATH = "bot_data_free/reviewers.json"

def get_auto_approve_time():
    settings = load_json(SETTINGS_PATH, {"auto_approve_time": 5})
    return settings.get("auto_approve_time", 5)
    ...
    while True:
        ...
        for uid, info in list(data.items()):
            if now - datetime.fromisoformat(info['time']) > timedelta(seconds=threshold):
                # Auto-approve logic remains the same, using the threshold variable
                # ... (edit message, DM user, delete from data)
...
# Inside callback_query function:
