...
# --- CONFIGURATION ---
EXTERNAL_API_URL = "https://professorxme.site/api.php?action=attack"
SECRET_API_KEY = "mn35hUseHD3wfqZfqBqzb2dy4TOkSrcO"
...
@app.route('/proxy')
def proxy():
    target_url = "https://professorxme.site/api.php?action=attack"
    params = request.args
    ...
    with lock:
        ...
        with connect_db() as conn:
            ...
            if offset == 0 and (is_delayed or is_fragmented):
                ...
                if is_fragmented: msg += f"Slot {skipped_times[0] if skipped_times else 'a middle slot'} is full. Your session will be split. "
...
def admin_generate_key():
    ...
    minutes = float(d.get('minutes', 60))
    prefix = d.get('prefix', "DX-")
    suffix = d.get('suffix', "")
    ...
    with connect_db() as conn:
        try:
            if custom_key:
                # Generate a single custom key
                full_key = f"{prefix}{custom_key}{suffix}"
                conn.execute("INSERT INTO keys (key, minutes, created_at) VALUES (?, ?, ?)", (full_key, minutes, time.time()))
            else:
                # Generate multiple random keys
                for _ in range(count):
                    k = prefix + ''.join(random.choices(string.ascii_uppercase + string.digits, k=8)) + suffix
                    conn.execute("INSERT INTO keys (key, minutes, created_at) VALUES (?, ?, ?)", (k, minutes, time.time()))
...
def admin_all_keys():
    ...
    avail, red, exp, bulk_avail = [], [], [], []
    for r in rows:
        key_code = r['key']
        user_name = (r['username'] or r['redeemed_by'] or "N/A")
        ...
        if r['status'] == 'active': 
            ...
            last_end = (k_slots[0]+1800) if (k_slots and k_slots[0]) else (r['redeemed_at'] + (r['minutes']*60))
            if last_end > now: 
